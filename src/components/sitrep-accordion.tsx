...
function rowStrength(row: SitRepRow): number {
  return row.taskGroups.reduce(
    (sum, tg) => sum + tg.units.reduce((s, u) => s + u.strength, 0),
    0
  );
}

function SitRepEntry({ row }: { row: SitRepRow }) {
  const totalStrength = rowStrength(row);
  return (
    <AccordionItem value={row.id}>
      <AccordionTrigger>
        <div className="flex flex-1 flex-wrap items-center justify-between gap-x-3 gap-y-1 pr-2 text-left text-sm">
          <span className="font-medium">{row.reportedAt.toLocaleString()}</span>
          <span className="text-xs text-muted-foreground">
            {totalStrength.toLocaleString()} strength · {row.taskGroups.length} task group
            {row.taskGroups.length === 1 ? "" : "s"}
          </span>
        </div>
      </AccordionTrigger>
      <AccordionContent>
        <div className="flex flex-col gap-4 text-sm">
          ...
          {row.significantActivities.length > 0 && (
            <div>
              <div className="mb-1 text-xs text-muted-foreground">Other Significant Activities</div>
...
export function SitRepAccordion({ rows }: { rows: SitRepRow[] }) {
  ...
  const byJtf = new Map<string, SitRepRow[]>();
          ...
          <AccordionItem key={jtfName} value={jtfName}>
            <AccordionTrigger>
              <div className="flex flex-1 flex-wrap items-center justify-between gap-x-3 gap-y-1 pr-2 text-left text-sm">
                ...
                <span className="text-xs text-muted-foreground">reports: {jtfRows.length}</span>
              </div>
            </AccordionTrigger>
            <AccordionContent>
              <div className="border-l border-border pl-3">
                <Accordion multiple>
                  {jtfRows.map((row) => (
                    <SitRepEntry key={row.id} row={row} />
                  ))}
                </Accordion>
              </div>
            </AccordionContent>
